...
assemblyinfo :global_version do |asm|
  ...
  asm.description = "Bitlore, minimalist validation library"
...
task :compile => [:versioning, :global_version, :build] do
	copyOutputFiles File.join(props[:src], "Bitlore/bin/Release"), "Bitlore.{dll,pdb,xml}", File.join(props[:output], 'net-portable')
end
...
xunit :tests => [:compile] do |xunit|
    xunit.command = File.join('src', 'packages','xunit.Runners.1.9.2', 'tools', 'xunit.console.clr4.exe')
    xunit.options = ["/xml #{File.join(props[:artifacts], 'xunit-test-results.xml')}"]
    xunit.assemblies = FileList[File.join(props[:src], "Bitlore.Tests/bin/Release", "Bitlore.Tests.dll")]
end
...
zip :zip_output => [:versioning] do |zip|
	...
	zip.output_file = "Bitlore-#{NUGET_VERSION}.zip"
...
msbuild :nuget_restore do |msb|
  ...
  msb.solution = 'src/Bitlore.Tests/Bitlore.Tests.csproj'
end

desc "Builds the nuget package"
task :nuget => [:versioning, :create_nuspec] do
	sh "src/.nuget/nuget.exe pack #{props[:artifacts]}/Bitlore.nuspec /Symbols /OutputDirectory #{props[:artifacts]}"
end
...
nuspec :_nuspec do |nuspec|
  ...
  nuspec.output_file = File.join(props[:artifacts], 'Bitlore.nuspec')
  add_files props[:output], 'Bitlore.{dll,pdb,xml}', nuspec
  nuspec.file(File.join(props[:src], "Bitlore\\**\\*.cs").gsub("/","\\"), "src")
end
